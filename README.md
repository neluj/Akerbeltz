<div align="center">
  <img src="img/readme-logo.png" alt="Xake logo" width="280">
  <h3>Xake</h3>
</div>

# Overview
I started this project out of interest in putting key techniques into practice in a chess engine: performance, efficiency, and strategy. My goal has been to progress incrementally, implementing and documenting each improvement to measure its impact gradually, rather than building a “powerful” engine from day one.
...
<table>
  <thead>
    <tr>
      ...
      <th>Version</th>
      <th>Elo</th>
      <th>Xake-1.0.0 vs (W-L-D)</th>
      <th>Score</th>
      <th>Elo diff (Xake-1.0.0 +/-)</th>
    </tr>
  </thead>
  <tbody>
    <tr>
...
Based on these matches, **Xake-1.0.0 lands around ~1615 Elo for this specific pool and time control**, which is the key takeaway from the table.

## Build and run

Optional CPU tuning: by default builds are generic. You can pass `-DXAKE_ARCH=native` (or e.g. `-DXAKE_ARCH=x86-64-v3`) to add `-march=<value>`.
...
- Configure and build (Release by default):
  ...
  cmake --build build
  ```
- Run the engine (UCI). The binary is named `Xake-<version>` (default `Xake-1.0.0`, override with `-DXAKE_ENGINE_VERSION=...`):
  ```bash
  ./build/Xake-1.0.0
  ```
- Tests are OFF by default; enable them with `-DXAKE_BUILD_TESTS=ON` when configuring (GoogleTest is fetched automatically).
  ```bash
  cmake -S . -B build -DXAKE_BUILD_TESTS=ON
  cmake --build build
  ```
- Run tests:
  ```bash
...
- Configure and build (Release by default):
  ...
  cmake --build build
  ```
- Run the engine (UCI). The binary is named `Xake-<version>.exe` (default `Xake-1.0.0.exe`, override with `-DXAKE_ENGINE_VERSION=...`):
  ```bash
  ./build/Xake-1.0.0.exe
  ```
- Tests are OFF by default; enable them with `-DXAKE_BUILD_TESTS=ON` when configuring:
  ```bash
  cmake -S . -B build -G Ninja -DXAKE_BUILD_TESTS=ON DCMAKE_CXX_COMPILER=x86_64-w64-mingw32-g++
  cmake --build build
  ```
- Run tests:
  ```bash
...
- Arena (Windows/Linux):
  ...
  3. Select `Xake-<version>` (Linux) or `Xake-<version>.exe` (Windows) from your build folder.
  4. When prompted, choose UCI as the protocol.
  5. Configure options like `Hash` under `Engines → Manage...` if needed.
  6. Start a new game and select Xake as the engine.
- Cute Chess (Linux/Windows/macOS):
  1. Open Cute Chess and go to the engine manager.
  2. Add a new engine and point it to `Xake-<version>` (Linux/macOS) or `Xake-<version>.exe` (Windows).
  3. Select UCI as the protocol and save the engine.
  4. Start a game or tournament and pick Xake as one of the engines.
- Other UCI GUIs (e.g., Banksia, SCID vs. PC) are available on Linux/macOS/Windows:
  1. Add a new engine and point it to `Xake-<version>` (Linux/macOS) or `Xake-<version>.exe` (Windows).
...
- Usage example:
  ```bash
  python3 scripts/perft_suite.py --engine ./build/Xake-1.0.0 \
...
//TODO etiquietas como en stockfish
//TODO añadir algun gif al readme para ver cómo utilizar? y alguo mas a la parte grafica
//TODO índice/sumario
//TODO se puede hacer un readme con los capítulos desplegables?
